...
{
    /* For platforms without timer IRQ support (like Phytium Pi), 
     * we still need to wait on the notification to maintain proper test flow.
     * The timer driver will send signals even in no-IRQ mode.
     */
    printf("Timer wait: Waiting on notification (compatibility mode)\n");
    seL4_Wait(env->timer_notification.cptr, NULL);
    printf("Timer wait: Notification received\n");
}
