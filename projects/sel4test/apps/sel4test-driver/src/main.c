...
/* ammount of untyped memory to reserve for the driver (8mb) */
#define DRIVER_UNTYPED_MEMORY (1 << 25)  // 32MB for Phytium Pi
/* Number of untypeds to try and use to allocate the driver memory.
 * if we cannot get 8mb with 16 untypeds then something is probably wrong */
#define DRIVER_NUM_UNTYPEDS 16

/* dimensions of virtual memory for the allocator to use */
#define ALLOCATOR_VIRTUAL_POOL_SIZE ((1 << seL4_PageBits) * 50)

/* static memory for the allocator to bootstrap with */
#define ALLOCATOR_STATIC_POOL_SIZE ((1 << seL4_PageBits) * 40)  // 40 pages for Phytium Pi
...
static void init_env(driver_env_t env)
{
    ...
    printf("Creating VKA interface\n");
    allocman_make_vka(&env->vka, allocman);
    printf("VKA interface created successfully\n");
    ...
    printf("Getting BootInfo for VSpace initialization\n");
    ...
    printf("Getting page directory from simple interface\n");
    seL4_CPtr pd_cap = simple_get_pd(&env->simple);
    if (pd_cap == seL4_CapNull) {
        ZF_LOGF("Failed to get page directory capability from simple interface");
    }
    printf("Page directory capability: %lu\n", pd_cap);
    ...
    printf("VSpace bootstrapped successfully\n");

    /* fill the allocator with virtual memory */
    printf("Reserving virtual memory pool of size: %d bytes\n", ALLOCATOR_VIRTUAL_POOL_SIZE);
    void *vaddr;
    virtual_reservation = vspace_reserve_range(&env->vspace,
                                               ALLOCATOR_VIRTUAL_POOL_SIZE, seL4_AllRights, 1, &vaddr);
    if (virtual_reservation.res == 0) {
        printf("Failed to provide virtual memory for allocator, vaddr: %p\n", vaddr);
        ZF_LOGF("Failed to provide virtual memory for allocator, vaddr: %p", vaddr);
    }
    printf("Virtual memory reserved successfully at %p\n", vaddr);

    printf("Configuring virtual pool for allocator\n");
    bootstrap_configure_virtual_pool(allocman, vaddr,
                                     ALLOCATOR_VIRTUAL_POOL_SIZE, pd_cap);
...
static unsigned int allocate_untypeds(vka_object_t *untypeds, size_t bytes, unsigned int max_untypeds)
{
    ...
    ZF_LOGI("=== Starting allocate_untypeds ===");
    ZF_LOGI("Target bytes: %zu, max_untypeds: %u", bytes, max_untypeds);

    /* try to allocate as many of each possible untyped size as possible */
    for (uint8_t size_bits = 28; size_bits > PAGE_BITS_4K; size_bits--) {
        int allocations_this_size = 0;
        ...
        while (num_untypeds < max_untypeds &&
            ...
            allocations_this_size++;
            num_untypeds++;
        }
        if (allocations_this_size > 0) {
            ZF_LOGI("Allocated %d untypeds of size %u bits (%zu bytes each)", 
                    allocations_this_size, size_bits, BIT(size_bits));
        }
    }
    
    ZF_LOGI("Total allocated: %u untypeds, %zu bytes", num_untypeds, allocated);
    ZF_LOGI("=== allocate_untypeds completed ===");
    return num_untypeds;
}
...
static unsigned int populate_untypeds(vka_object_t *untypeds)
{
    ...
    ZF_LOGI("Reserving %zu bytes (%u MB) for driver with max %u untypeds", 
            DRIVER_UNTYPED_MEMORY, DRIVER_UNTYPED_MEMORY >> 20, DRIVER_NUM_UNTYPEDS);
    ...
    ZF_LOGI("Reserved %u untypeds for driver", reserve_num);

    /* Now allocate everything else for the tests */
    ZF_LOGI("Allocating untypeds for tests (max %zu bytes, max %zu objects)", 
            (size_t)(1 << 26), ARRAY_SIZE(untyped_size_bits_list));
    ...
    ZF_LOGI("Freeing %u reserved objects", reserve_num);
    ...
    ZF_LOGI("=== populate_untypeds completed: %u untypeds available ===", num_untypeds);
    return num_untypeds;
}
...
static void init_timer(void)
{
    if (config_set(CONFIG_HAVE_TIMER)) {
        ...
        /* Allocate the timer notification if not already done (for platforms without IRQs) */
        if (env.timer_notification.cptr == seL4_CapNull) {
            error = vka_alloc_notification(&env.vka, &env.timer_notification);
            ZF_LOGF_IF(error, "Failed to allocate timer notification object");
        }
...
void sel4test_stop_tests(test_result_t result, int tests_done, int tests_failed, int num_tests, int skipped_tests)
{
    ...
    /* Account for skipped tests: tests_done should equal num_tests (enabled tests) */
...
void *main_continued(void *arg UNUSED)
{
    ...
    printf("=== main_continued started ===\n");
    printf("env.vspace pointer: %p\n", &env.vspace);
    printf("env.vka pointer: %p\n", &env.vka);
    printf("env.vspace content check:\n");
    printf("  env.vspace.data: %p\n", env.vspace.data);
    printf("  env.vspace.new_pages: %p\n", env.vspace.new_pages);
    printf("  env.vspace.reserve_range_at: %p\n", env.vspace.reserve_range_at);
    printf("Validating vspace structure...\n");
    if (env.vspace.data == NULL) {
        printf("ERROR: env.vspace.data is NULL!\n");
    }
    if (env.vspace.new_pages == NULL) {
        printf("ERROR: env.vspace.new_pages function pointer is NULL!\n");
    }
    if (env.vspace.reserve_range_at == NULL) {
        printf("ERROR: env.vspace.reserve_range_at function pointer is NULL!\n");
    }
    ...
    printf("About to populate untypeds...\n");
    env.num_untypeds = populate_untypeds(untypeds);
    env.untypeds = untypeds;
    printf("Untypeds populated: %u\n", env.num_untypeds);

    /* create a frame that will act as the init data, we can then map that
     * in to target processes */
    printf("About to allocate init data frame, vspace at: %p\n", &env.vspace);
    printf("Before vspace_new_pages call:\n");
    printf("  env.vspace.data: %p\n", env.vspace.data);
    printf("  env.vspace.new_pages: %p\n", env.vspace.new_pages);
    printf("  Calling vspace_new_pages(&env.vspace=%p, seL4_AllRights, 1, PAGE_BITS_4K)\n", &env.vspace);
    
    if (&env.vspace == NULL) {
        printf("ERROR: env.vspace is NULL!\n");
        ZF_LOGF("env.vspace is NULL!");
    }
    if (env.vspace.data == NULL) {
        printf("ERROR: env.vspace.data is NULL before vspace_new_pages!\n");
        ZF_LOGF("env.vspace.data is NULL before vspace_new_pages!");
    }
    
    env.init = (test_init_data_t *) vspace_new_pages(&env.vspace, seL4_AllRights, 1, PAGE_BITS_4K);
    assert(env.init != NULL);
    printf("Init data frame allocated at: %p\n", env.init);
...
int main(void)
{
    ...
    ZF_LOGI("=== seL4 Test Driver Starting on Phytium Pi ===");
    ZF_LOGI("BootInfo pointer: %p", info);
    if (info) {
        ZF_LOGI("Initial BootInfo - nodeID: %u, numNodes: %u, numIOPTLevels: %u",
                info->nodeID, info->numNodes, info->numIOPTLevels);
        ZF_LOGI("Initial BootInfo - extraLen: %u, initThreadCNodeSizeBits: %u",
                info->extraLen, info->initThreadCNodeSizeBits);
    } else {
        ZF_LOGF("BootInfo is NULL at main entry - critical failure!");
    }
    ...
    ZF_LOGI("Simple interface initialized successfully");

    /* initialise the test environment - allocator, cspace manager, vspace
     * manager, timer
     */
    ZF_LOGI("Starting test environment initialization");
    init_env(&env);
    ZF_LOGI("Test environment initialized successfully");
